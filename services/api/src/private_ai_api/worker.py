...
pure Python, LightRAG chunks with tiktoken, merges a networkx graph and rewrites its KV
store -- so it holds the GIL for as long as the file takes. Sharing a process with the API
therefore means sharing a stalled event loop: uploads, chat and even a plain SQLite read
queue behind whatever file is being read. ``asyncio.to_thread`` does not help, because the
GIL is what is contended, not a thread.

This process owns that work instead. It takes documents off the queue the API writes to,
using the same ``document_claims`` rows that already keep two processes from ingesting the
same document, and the API keeps its loop for HTTP.
"""
...
logger = logging.getLogger("private_ai_api.worker")
...
async def wait_for_schema(settings: Settings, stop: asyncio.Event) -> bool:
    """Block until the API has created the tables, or until asked to stop.

    Whoever starts first is not ours to control -- the dev script launches both at once --
    so the worker waits rather than racing the migrations or crashing on a missing table.
    """
...
async def run_worker(settings: Settings, stop: asyncio.Event) -> None:
    ...
    if not await wait_for_schema(settings, stop):
        return
    # The API owns the schema: it is the process the user starts first and the one whose
    ...
    try:
        ...
        await services.memory_service.sync_all()
        while not stop.is_set():
            try:
                await asyncio.wait_for(stop.wait(), timeout=settings.worker_poll_seconds)
                break
            ...
            if await asyncio.to_thread(_pending_count, services):
                await processor.process_pending(recover=False)
