...
class WebSearchService:
    ...
    async def run(
        ...
        *,
        ...
        text = query.strip()
        if not text:
            raise WebSearchUnavailable("Search query cannot be empty")
        ...
        if config.backend == SEARXNG:
            ...
            raise WebSearchNotConfigured(f"Unsupported web search backend: {config.backend}")
    ...
    async def _searxng(
        ...
        if not base_url:
            raise WebSearchNotConfigured("Set the SearXNG address before searching the web")
        try:
            ...
            raise WebSearchUnavailable(f"SearXNG is not reachable: {exc}") from exc
        ...
        if not isinstance(rows, list):
            raise WebSearchUnavailable("SearXNG returned an unexpected payload")
        return [
            item
            for item in (_searxng_result(row) for row in rows[: count * 2])
            if item is not None
        ]
    ...
    async def _duckduckgo(self, query: str, count: int) -> list[WebSearchResult]:
        try:
            ...
            raise WebSearchUnavailable(f"DuckDuckGo is not reachable: {exc}") from exc
        ...
        if not parser.results:
            ...
            raise WebSearchUnavailable(
                "DuckDuckGo returned no results, which usually means it rate-limited this "
                "machine. Wait a moment, or switch to SearXNG."
            )
        return [
            WebSearchResult(
                title=row["title"],
    ...
    async def _openai(
        ...
        if not config.api_key.strip():
            raise WebSearchNotConfigured("Add an OpenAI API key before using OpenAI web search")
        payload = {
        ...
        try:
            ...
            raise WebSearchUnavailable(f"OpenAI web search is not reachable: {exc}") from exc
        except ValueError as exc:
            raise WebSearchUnavailable("OpenAI returned an unreadable response") from exc
...
def _openai_error(response: httpx.Response) -> str:
    try:
        ...
        detail = None
    return f"OpenAI web search failed ({response.status_code}): {detail or response.text[:200]}"
