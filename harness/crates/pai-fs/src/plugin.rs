//! Cắm hệ tệp vào cây.
//!
//! Một plugin, năm tool, một provider và một chính sách. Gỡ nó ra là mất cả năm tool
...
use crate::tools::{edit::Edit, glob::GlobTool, grep::Grep, read::Read, write::Write};
...
impl Plugin for FsPlugin {
    ...
    async fn apply(&self, ctx: &Context) -> anyhow::Result<()> {
        ...
        ctx.keep(tools.register(Arc::new(Grep::new(self.roots.clone()))));
