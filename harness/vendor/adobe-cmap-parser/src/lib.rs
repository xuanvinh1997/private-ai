// Cảnh báo của mã thượng nguồn, không phải của ta. Là dependency đường dẫn nên cargo
// không còn nuốt hộ nữa, và một cảnh báo lặp lại ở mọi lần build là một cảnh báo
// người ta học cách không đọc.
#![allow(non_snake_case)]

extern crate pom;

use pom::char_class::{hex_digit, oct_digit, multispace};
use pom::Parser;
use pom::parser::*;
use pom::DataInput;
...
/// Một byte được phép nằm trong một token PostScript trần.
///
/// PATCH (xem README.md): bản gốc chỉ nhận `alpha` cộng `*'"`, nên nó dừng ở dấu `-`
/// trong `/CMapName LiberationSerif-cmap def`. `file()` là một `repeat(1..)`, nên "dừng"
/// ở đây nghĩa là **cắt cụt cả dòng token trong im lặng**: bảng `beginbfchar` nằm ngay
/// sau đó không bao giờ được đọc, `get_unicode_map` trả về một `HashMap` rỗng kèm `Ok`,
/// và cả tài liệu rút ra toàn khoảng trắng mà không một lỗi nào được nêu.
///
/// Đây là luật regular character của PostScript (PLRM 3rd ed. §3.1): mọi thứ trừ khoảng
/// trắng và tám delimiter `()<>[]{}/%`. Nới đúng tới đó chứ không nới cho vừa một tệp.
fn regular(term: u8) -> bool {
    !matches!(
        term,
        b' ' | b'\t' | b'\r' | b'\n' | b'\0' | b'\x0C'
            | b'(' | b')' | b'<' | b'>' | b'[' | b']' | b'{' | b'}' | b'/' | b'%'
...
fn operator() -> Parser<u8, String> {
    is_a(regular).repeat(1..).convert(|v|String::from_utf8(v))
}
...
mod tests {
    ...
    /// Bảng `ToUnicode` của một PDF do Calibre sinh ra, rút gọn còn hai lối vào.
    ///
    /// Dấu gạch ngang trong `LiberationSerif-cmap` là toàn bộ nội dung của bài kiểm này:
    /// với bản gốc, bộ lexer dừng ở đó và `beginbfchar` bên dưới không bao giờ được đọc,
    /// nên hàm trả về `Ok` kèm một bảng **rỗng** — không lỗi, không hoảng loạn, và cả
    /// tài liệu rút ra toàn khoảng trắng.
    const CALIBRE: &'static [u8] = b"/CIDInit /ProcSet findresource begin
12 dict begin
begincmap
...
";
    ...
    #[test]
    fn ten_cmap_co_dau_gach_ngang() {
        let map = get_unicode_map(CALIBRE).unwrap();
        assert_eq!(map.len(), 2);
        assert_eq!(map.get(&0x44), Some(&vec![0x00, 0x61]));
        assert_eq!(map.get(&0x48), Some(&vec![0x00, 0x65]));
    }
}
